...
% This script is used to reproduce Figure 7i and 7j within the research
...
% In the folder: Sensitivity_Analysis\Inputs\SSIV_VideoA - all inputs in
...
% Alter the inputs
dirName = 'F:\Year 3\BelgradeData_toUpload\ToUpload\Sensitivity_Analysis\Inputs\SSIV_VideoA\';
%
...
for aa = 1:length(fileNameIn)
    ...
    xIn = 20; % This is the x- axis co-ordinates. Outputs at 10 and 20m
    leftBank = 25; % in y
    rightBank = 5; % in y
    stepDist = 0.5;
    
    %OTV inputs
    allXY = [ssivXtransGrid(:),ssivYtransGrid(:)]; % exported x y grid for PTV
    uOut = normalVel; % PTV out
