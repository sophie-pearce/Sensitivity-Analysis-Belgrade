...
% Outputs: 
% This script will produce the velocities and locations of the OTV surface
% velocities, in comparisons to the ADCP measurements. 

%Inputs: 
% You will need the OTV data for each video (to be run seperately)
...
dirName = 'F:\Year 3\BelgradeData_toUpload\ToUpload\ADCP_Comparisons\OTV\';
...
for aa = 1
    ...
    stepDist = 0.5;
    allXY = [otvXtransGrid(:), optvYtransGrid(:)]; % exported x y grid for PIV
    uOut = zi; % PIV out
    uOut1 = uOut(:);
