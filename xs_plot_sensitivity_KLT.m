...
% This script is used to reproduce Figure 7a and 7b within the research
...
% In the folder: Sensitivity_Analysis\Inputs\KLT_VideoA - all inputs in
...
% Alter the inputs
dirName = 'F:\Year 3\BelgradeData_toUpload\ToUpload\Sensitivity_Analysis\Inputs\KLT_VideoA\';
...
for aa = 1:length(fileNameIn)
    ...
    bb = regress([xIn(1), xIn(1)]',[leftBank, rightBank; 1, 1]'); %[x,y]
    interogation = rightBank:stepDist:leftBank; % y-axis (channel width)
    xExtract = bb(2)+interogation.*bb(1)'; %x-axis (up/downstream)
    
    U = xyzA_final(:,1);
    V = xyzA_final(:,2);
    requiredResolution = 0.5; % KLT vectors are gridded to this value in meters
    xi = 0:requiredResolution:35;
    yi = -15:requiredResolution:45;
    ...
    uOut = zi; % KLT velocity (gridded and averaged)
