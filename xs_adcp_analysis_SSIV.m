...
% Outputs: 
% This script will produce the velocities and locations of the SSIV surface
% velocities, in comparisons to the ADCP measurements. 

%Inputs: 
% You will need the SSIV data for each video (to be run seperately)
...
dirName = 'F:\Year 3\BelgradeData_toUpload\ToUpload\ADCP_Comparisons\SSIV\';
...
for aa = 1
    ...
    %PIV inputs
    allXY = [ssivXtransGrid(:), ssivYtransGrid(:)]; % exported x y grid for PIV
